...
class NarratorManifestations {
    ...
    update(ctx, camera, storyEngine) {
        this.updateNarratorEyes(ctx, camera, storyEngine);
        this.updateMemoryEchoes(ctx, camera);
    }
    
    updateNarratorEyes({ dt, t }, camera, storyEngine) {
            ...
            try {
                ...
                if (distanceToPlayer < 15 && storyEngine && storyEngine.playerData) {
                    ...
                    if (relationship > 30 || relationship < -30) {
                        ...
                        eye.userData.attention = Math.min(1, (eye.userData.attention || 0) + dt);
                        ...
                        eye.userData.attention = Math.max(0, (eye.userData.attention || 0) - dt);
                    }
                }
                
                eye.userData.blinkTimer = (eye.userData.blinkTimer || 0) - dt;
                ...
                if (eye.position) {
                    eye.position.y += Math.sin(t + eye.position.x) * 0.01;
                }
            } catch (error) {
        ...
        this.memoryEchoes.forEach(echo => {
            ...
            try {
                ...
                if (echo.userData.decision && echo.rotation && echo.position) {
                    echo.rotation.y += dt * (1 + echo.userData.emotionalResonance);
                    echo.position.y = echo.userData.originalPosition.y + 
                        Math.sin(t * 2 + echo.position.x) * (0.2 + echo.userData.emotionalResonance * 0.3);
                    
                    const pulse = Math.sin(t * 3) * (0.1 + echo.userData.emotionalResonance * 0.2);
